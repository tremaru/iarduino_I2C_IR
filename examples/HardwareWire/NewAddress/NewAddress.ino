// ПРИМЕР СМЕНЫ АДРЕСА МОДУЛЯ:                    // * Строки со звёздочкой являются необязательными.
                                                  //
// ИК-приемник/передатчик (Роботраффик):          //   https://iarduino.ru/shop/Expansion-payments/ik-priemnik-peredatchik-flash-i2c.html
...
// и вывод текущего адреса модуля на шине I2C.    //
                                                  //
// Библиотека <iarduino_I2C_IR.h> предназначена   //
// для работы с ИК-приёмником/передатчиком        //
// серии Flash I2C и меняет только их адреса.     //
...
iarduino_I2C_IR ir;                               //   Объявляем объект ir   для работы с функциями и методами библиотеки iarduino_I2C_IR.
...
void setup(){                                     //
    ...
    if( ir.begin(&Wire) ){                        //   Инициируем работу с модулем, указав ссылку на объект для работы с шиной I2C на которой находится модуль (Wire, Wire1, Wire2 ...).
